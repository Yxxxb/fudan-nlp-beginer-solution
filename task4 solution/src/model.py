...
# Model constructure code is partly borrowed from the repo https://github.com/threelittlemonkeys/lstm-crf-pytorch.
...
class CRF(nn.Module):
    ...
    def predict(self, scores, mask):
        ...
        for i in range(1, sequence_len):
            ...
            temp_tag = torch.cat([torch.tensor(range(num_tags)).view(1, -1, 1) for _ in range(batch_size)],
                                 dim=0).cuda()
